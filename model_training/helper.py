...
def upsample_and_one_hot_encode_train_data(train_data):
    """
    ...
    # resampling to balance the minority and majority class
    x_oversampled, y_oversampled = resample(x_train[y_train == 1], y_train[y_train == 1],
                                            replace=True, n_samples=x_train[y_train == 0].shape[0], random_state=4262)
    x_balanced = pd.concat([x_train, x_oversampled])
    y_balanced = pd.concat([y_train, y_oversampled])
    y_train = y_balanced
    ...
    ct.fit(x_balanced) # fit column transformer on train x
    X_train = ct.transform(x_balanced) # One-hot encoded X_train matrix
